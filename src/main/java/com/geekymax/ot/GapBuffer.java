...
class GapBuffer implements CharSequence, Serializable {
    ...
    public synchronized String subSequence(int start, int end) {
        ...
        // all data lay left side of the gap
        if (start < lo && end < lo) {
            arraycopy(text, start, res, 0, end - start);
            // all data lay right size of the gap
        } else if (start >= lo) {
    ...
    public synchronized void insert(int pos, String subStr) {
        System.out.println("gapBuffer insert" + pos + ";" + subStr);
        if (pos < 0) {
